...
const script = async () => {
  // Canvas setup
  const canvas = document.getElementById(('ComputeShaderCanvas'));
  ...
  const computeShaderSource = `#version 310 es
    ...
    void main() {
      ...
      //ssbo.pos[threadIndex] = vec2(floatIndex * 0.25 - 0.875, 0.5 * sin(time * 0.02 + floatIndex * 0.5)); oryginal
      ssbo.pos[threadIndex] = vec2( 0.5 * sin(time * 0.02 + floatIndex * 0.5)*0.1, -(time * 0.01 - 0.875));
    }
    `;
